...
function sessionCheck(request, response, next) {
    console.log('sessionCheck - Comprobando la sesion del usuario: ' + request.session.user );
		...
		if (user) {
			...
			newUser.save(function(err) {
		        ...
		        if (!err) {
					...
					//return response.status(200).send(newUser.username);  //ANTES enviaba string
					return response.status(200).send({username: newUser.username}); //Enviar objeto JSON en vez de solo String
					//return response.status(200).send(newUser); //ANGULAR 5 (no quiero enviar password, etc)
			        ...
			        request.session.regenerate(function() { 
						...
						console.log('El usuario creado es: ' + newUser.username);
						request.session.user = newUser.username;
						
						
						console.log("VALOR request.session: " );
						console.log(request.session);
						console.log("VALOR request.sessionID: "     + request.sessionID);
						console.log("VALOR request.session.user: "  + request.session.user);
    ...
    User.findOne({username: username}, function(err, user) {
		...
		if (isValidPassword(user, password)) { 
	        ...
	        request.session.regenerate(function() {  //cannot read property "regenerate" of undefined
				console.log('Entra en regenerate para la sesion en el LOGIN');
				console.log('El usuario con el que se ha logeado es: ' + username);
				request.session.user = username;
				...
				console.log("VALOR request.sessionID: "     + request.sessionID);
				console.log("VALOR request.session.user: "  + request.session.user);
				console.log("VALOR request.sessionStorage: " + request.sessionStorage);
